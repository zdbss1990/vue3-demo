 ...
 * @LastEditTime: 2022-09-13 14:05:37
 */
import {
    setStorage,
    getStorage
   ...
   mutations:{
    ...
    FILTER_ROUTES_LIST(state,route){
        if(route.flag=='default'){
            state.routerList=state.routerList.filter(item=>item.name!==route.currentRoute.name)
        }else if(route.flag=='all'){
            state.routerList=state.routerList.filter(item=>item.name == route.currentRoute.name)
        }else{
            state.routerList=state.routerList.filter(item=>item.name == route.currentRoute.name || item.name =='Home')
        }
    }
   },
   actions:{
